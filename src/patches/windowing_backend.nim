...
proc setWindowingBackend*(backend: WindowingBackend) =
  if getState("windowing_backend", WindowingBackend, autodetectWindowingBackend()) == backend:
  ...
  if backend == WindowingBackend.Wayland:
    ...
    if execCmd(pkexec & "flatpak override --nofilesystem=/tmp/.X11-unix " & SOBER_APP_ID) != 0:
    ...
    if execCmd(pkexec & "flatpak override --filesystem=/tmp/.X11-unix " & SOBER_APP_ID) != 0:
      error "lucem: failed to give Sober access to the X11 socket!"
      return
    
    debug "lucem: restricting Wayland socket permissions from Sober"
    if execCmd(pkexec & "flatpak override --no-talk-name=org.freedesktop.Platform.Wayland " & SOBER_APP_ID) != 0:
